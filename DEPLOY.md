...
```bash
(crontab -l 2>/dev/null | grep -v "perf_commissions" ; echo "0 * * * * /opt/perf_commissions/run_server.sh") | crontab -
```
...
Attendu : `0 * * * * /opt/perf_commissions/run_server.sh`
...
| Heure | Comportement |
|-------|-------------|
| 00:00 | Cron lance `run_server.sh` → `git pull` + `run_daily.py` |
| 00:00 | `run_daily.py` verifie `last_success.txt` → pas de flag → lance le pipeline |
| 00:00 | Si pas de mail : le script echoue proprement, email d'alerte envoye |
| 01:00 | Cron relance → toujours pas de flag → reessaie |
| ...   | Repeat jusqu'a ce que le mail d'Abraham arrive |
| XX:00 | Mail trouve → pipeline complet → `last_success.txt` = date du jour |
| XX+1  | Cron relance → flag du jour existe → exit 0 instantanement |
